...
- **React Features (Strict Requirement):**
  ...
  - **Local/global UI state** (form-adjacent state, modal open/closed, active tab) uses plain `useState`/`useReducer` for component-local state, and **Zustand** for the rare cases that need to be shared across distant components (e.g. `addBabyDialog.store.ts`, opened from both the top nav and the profile page). Don't reach for Zustand for state a single component tree already owns via props. There is deliberately no global "currently-selected baby" store — nav lives in a top bar (not a baby-scoped sidebar), every screen shows the whole household at once (each item/card tagged with a small colored avatar via `babyAvatarAppearance`/`babyInitials`), and "which baby" is only ever asked inside a creation dialog's own local state (`BabyPickerStep`, skipped entirely for single-child households).
...
- **Silent refresh via `/auth/refresh`:** the backend now exposes `POST /auth/refresh`, which reads the `refresh_token` cookie and rotates both `access_token`/`refresh_token` cookies. On a `401`, the HTTP client should attempt exactly one call to `/auth/refresh` and retry the original request once; if the refresh call itself fails (e.g. its own `401`, meaning the refresh token is expired/invalid), clear identity state and redirect to `/login`. Never retry more than once per request — no silent retry loops.
- **HTTP client:** the shared fetch wrapper in `lib/` ALWAYS sends `credentials: 'include'`, and attaches a CSRF header if the backend requires one alongside the cookie. The single-refresh-then-redirect behavior above should live in one place (e.g. a TanStack Query global error handler or a fetch wrapper interceptor), not duplicated per feature.
