...
export function MilestoneTimelineSkeleton() {
  return (
    <div className="relative ml-4 max-w-3xl sm:ml-8" aria-hidden="true">
      <div className="absolute top-6 bottom-6 left-6 w-1 rounded-full bg-slate-100" />

      <div className="space-y-10">
        {[0, 1, 2].map((index) => (
          <div key={index} className="relative flex items-start">
            <Skeleton className="absolute left-6 z-10 -ml-[1.65rem] h-14 w-14 rounded-2xl" />
            <div className="ml-20 w-full space-y-3 rounded-[2rem] border border-slate-100 bg-white p-8 shadow-sm">
