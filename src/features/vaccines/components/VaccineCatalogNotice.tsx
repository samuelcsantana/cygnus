...
export function VaccineCatalogNotice({ metadata, className }: VaccineCatalogNoticeProps) {
  ...
  return (
    <aside className={cn('rounded-2xl border border-sky-100 bg-sky-50/70 p-4 text-sm', className)}>
      <p className="font-bold text-sky-900">{t('vaccines.catalog.title')}</p>
      ...
      <p className="mt-2 text-xs leading-relaxed text-sky-800">{t('vaccines.catalog.disclaimer')}</p>
    </aside>
  )
}
