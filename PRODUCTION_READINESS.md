...
- [x] **Zero observabilidade em produção** — Error Boundary global + `@sentry/react` (`src/lib/error-reporting.ts`), agora com projeto Sentry real e `VITE_SENTRY_DSN` preenchido em `.env.production`. **Deixou de ser no-op:** um erro não capturado disparado no build de produção (`vite build` + `vite preview`) foi aceito pelo endpoint de ingestão com HTTP 200 e id de evento `fe5ae009f05245e7b6132de91f64ce20` — id retornado significa evento armazenado, não só requisição aceita.
  - **Ressalva honesta:** isso foi validado em **build de produção**, não em produção — o app ainda não está publicado em lugar nenhum (só o Storybook vai pro GitHub Pages). Ver o item de HTTPS/domínio acima; enquanto ele não fechar, o Sentry não tem tráfego real pra observar.
  - **Falta ainda:** source maps no build (stack trace legível em vez de minificado — exige auth token, esse sim secreto), Web Vitals como métrica, e alerta configurado por taxa de erro.
